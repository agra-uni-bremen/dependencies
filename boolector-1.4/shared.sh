...
unpack() {
  ...
  cd $build_dir &&
  patch -p1 < $patches_dir/picosat_include_path.patch &&
  patch -p1 < $patches_dir/0001*.patch &&
  patch -p1 < $patches_dir/0003*.patch &&
  patch -p1 < $patches_dir/0004*.patch &&
...
pre_build() {
  ...
  sed -i 2s/Boolector_VERSION/"Boolector_VERSION $version"/ $build_dir/CMakeLists.txt
}
